...
from net.Transformer_GRU import Transformer_GRU
from net.Transformer_LSTM import Transformer_LSTM
from net.Transformer_RNN import Transformer_RNN
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--net-unit", type=str, default="GRU", help="select GRU, LSTM, and RNN")
    parser.add_argument(
        "--win-rate",
        type=float,
        default=1.0,
        help="the expected winning rate: Optimal policy can get 0.8",
    )
    parser.add_argument(
        "--watch",
        default=False,
...
def get_agents(
    ...
    if agent_learn is None:
        # model
        if args.net_unit == "RNN":
            net = Transformer_RNN(
            state_shape=args.state_shape,
            action_shape=args.action_shape,
            device=args.device
            ).to(args.device)
        elif args.net_unit == "LSTM":
            net = Transformer_LSTM(
            state_shape=args.state_shape,
            action_shape=args.action_shape,
            device=args.device
            ).to(args.device)
...
def train_agent(
    ...
    log_path = os.path.join(args.logdir, "tic_tac_toe", "ddqn_t_"+args.net_unit)
    ...
    def save_best_fn(policy):
        if hasattr(args, "model_save_path"):
            ...
            model_save_path = os.path.join(
                args.logdir, "tic_tac_toe", "ddqn_t_"+args.net_unit, "policy.pth"
            )
        torch.save(
